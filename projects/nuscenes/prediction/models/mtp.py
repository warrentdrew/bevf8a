...
class MTP(nn.Layer):
    ...
    def forward(self, image_tensor: paddle.Tensor,
                agent_state_vector: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        features = paddle.concat([backbone_features, agent_state_vector], axis=1)
        ...
        return paddle.concat((predictions, mode_probabilities), 1)
...
class MTPLoss:
    ...
    def _get_trajectory_and_modes(self,
                                  model_prediction: paddle.Tensor) -> Tuple[paddle.Tensor, paddle.Tensor]:
        """
    ...
    @staticmethod
    def _angle_between(ref_traj: paddle.Tensor,
                       traj_to_compare: paddle.Tensor) -> float:
        """
        ...
        if paddle.isnan(traj_to_compare[-1]).any() or paddle.isnan(ref_traj[-1]).any():
            return 180. - EPSILON

        traj_norms_product = float(paddle.linalg.norm(ref_traj[-1]) * paddle.linalg.norm(traj_to_compare[-1]))
    ...
    @staticmethod
    def _compute_ave_l2_norms(tensor: paddle.Tensor) -> float:
        """
        Compute the average of l2 norms of each row in the tensor.
        :param tensor: Shape [1, n_timesteps, 2].
        :return: Average l2 norm. Float.
        """
        l2_norms = paddle.linalg.norm(tensor, p=2, dim=2)
        avg_distance = paddle.mean(l2_norms)
        return avg_distance.item()

    def _compute_angles_from_ground_truth(self, target: paddle.Tensor,
                                          trajectories: paddle.Tensor) -> List[Tuple[float, int]]:
        """
    ...
    def _compute_best_mode(self,
                           ...
                           target: paddle.Tensor, trajectories: paddle.Tensor) -> int:
        """
    ...
    def __call__(self, predictions: paddle.Tensor, targets: paddle.Tensor) -> paddle.Tensor:
        """
        ...
        for batch_idx in range(predictions.shape[0]):
            ...
            best_mode_target = paddle.to_tensor([best_mode])
            ...
            if batch_losses is None:
                batch_losses = loss.unsqueeze(0)
            else:
                batch_losses = paddle.concat((batch_losses, loss.unsqueeze(0)), 0)

        avg_loss = paddle.mean(batch_losses)
