...
from projects.nuscenes.eval.common.render import setup_axis
from projects.nuscenes.eval.common.utils import boxes_to_sensor
from projects.nuscenes.eval.detection.constants import TP_METRICS, DETECTION_NAMES, DETECTION_COLORS, TP_METRICS_UNITS, \
    PRETTY_DETECTION_NAMES, PRETTY_TP_METRICS
from projects.nuscenes.eval.detection.data_classes import DetectionMetrics, DetectionMetricData, DetectionMetricDataList
from projects.nuscenes.utils.data_classes import LidarPointCloud
from projects.nuscenes.utils.geometry_utils import view_points
...
def class_pr_curve(md_list: dict,
                   metrics: dict,
                   detection_name: str,
                   min_precision: float,
                   min_recall: float,
    ...
    for dist_inteval in md_list.keys():
        data = md_list[dist_inteval].get_class_data(detection_name)
        # Plot the recall vs. precision curve for each distance threshold.
        for md, dist_th in data:
            md: DetectionMetricData
            ap = metrics[dist_inteval].get_label_ap(detection_name, dist_th)
            ax.plot(md.recall, md.precision, label='Dist. : {}, AP: {:.1f}'.format(dist_inteval, ap * 100))
...
def class_tp_curve(md_list: dict,
                   metrics: dict,
                   detection_name: str,
                   min_recall: float,
                   dist_th_tp: float,
...
def dist_pr_curve(md_list: dict,
                  metrics: dict,
                  dist_inteval: float,
                  min_precision: float,
                  min_recall: float,
    ...
    for detection_name in detection_names:
        data = md_list[dist_inteval].get_class_data(detection_name)
        for md, dist_th in data:
            md = md_list[dist_inteval][(detection_name, dist_th)]
            ap = metrics[dist_inteval].get_label_ap(detection_name, dist_th)
...
def summary_plot(md_list: dict,
    ...
    _, axes = plt.subplots(nrows=n_classes, ncols=1, figsize=(15, 5 * n_classes))
    for ind, detection_name in enumerate(class_names):
        #title1, title2 = ('Recall vs Precision', 'Recall vs Error') if ind == 0 else (None, None)
        title1 = ('Recall vs Precision') if ind == 0 else (None)
