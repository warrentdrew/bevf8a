...
def geo_iou_match(accumulate_dict: dict, 
                  gt_boxes: EvalBoxes, 
    ...
    for pred_box in pred_boxes_list:
        pred_translation = list(pred_box.translation)
        pred_size = list(pred_box.size)
        pred_yaw = quaternion_yaw(Quaternion(pred_box.rotation))  
        if pred_box.sample_token not in pred_dict.keys():
            pred_dict.update({pred_box.sample_token: [pred_translation+pred_size+[pred_yaw, pred_box.detection_score, pred_box.index]]})
        else:
            pred_dict[pred_box.sample_token].append(pred_translation+pred_size+[pred_yaw, pred_box.detection_score, pred_box.index])
        
    for sample_token in pred_dict.keys():
        gt_dict.update({sample_token: []})
        for gt_idx, gt_box in enumerate(gt_boxes[sample_token]):
            if cfg.ignore_gt_valid: 
                gt_box.valid = True 
            if gt_box.detection_name == class_name and gt_box.valid: 
                gt_translation = list(gt_box.translation)
                gt_size = list(gt_box.size)
                gt_yaw = quaternion_yaw(Quaternion(gt_box.rotation))
                gt_dict[sample_token].append(gt_translation+gt_size+[gt_yaw, gt_box.index])

    for sample_token in pred_dict.keys():
        pred_matrix = np.array(pred_dict[sample_token])
        gt_matrix = np.array(gt_dict[sample_token])

        if gt_matrix.shape[0] == 0:
            iou_metric  = np.ones((pred_matrix.shape[0], 2), dtype=pred_matrix.dtype)*np.inf
            dist_metric = np.ones((pred_matrix.shape[0], 2), dtype=pred_matrix.dtype)*np.inf
        else:
            sortind = np.argsort(-pred_matrix[:, 7])
            pred_matrix = pred_matrix[sortind, ...]
            
            # if gt_matrix.shape[0] > 1:
            #     gt_dist = np.linalg.norm(gt_matrix[:, :2], axis=1)
            #     sortind = np.argsort(gt_dist)
            #     gt_matrix = gt_matrix[sortind, ...]

            iou3d, ioubev = iou_calculate(pred_matrix[:,:7], gt_matrix[:, :7])
            iou3d = iou3d.unsqueeze(0).cpu().numpy()
            ioubev = ioubev.unsqueeze(0).cpu().numpy()
            iou_metric = 1-np.concatenate([iou3d, ioubev], axis=0)
            dist_metric = cfg.rel_dist_fcn_callable(pred_matrix[:,:7], gt_matrix[:,:7])

        if sample_token not in accumulate_dict['sample_token'].keys():
            accumulate_dict['sample_token'].update({sample_token: 
                                                   {class_name: {'pred_matrix': pred_matrix,
                                                                  'gt_matrix': gt_matrix,
            ...
            accumulate_dict['sample_token'][sample_token].update(
                                                    {class_name: {'pred_matrix': pred_matrix,
                                                                  'gt_matrix': gt_matrix,
                                                                  'iou_metric': iou_metric,
                                                                  'dist_metric': dist_metric,
...
def geo_iou_match_bi(npos: int, 
    ...
    for pred_box in pred_boxes_list:
        ...
        pred_size = list(pred_box.size)
        pred_yaw = quaternion_yaw(Quaternion(pred_box.rotation))  
        pred_score = pred_box.detection_score
        class_id = class_inds[pred_box.detection_name]
        if pred_box.sample_token not in pred_dict.keys():
            pred_dict.update({pred_box.sample_token: [pred_translation+pred_size+[pred_yaw, pred_score, class_id, pred_box.index]]})
        else:
            pred_dict[pred_box.sample_token].append(pred_translation+pred_size+[pred_yaw, pred_score, class_id, pred_box.index])
    ...
    for sample_token in pred_dict.keys():
        ...
        for gt_idx, gt_box in enumerate(gt_boxes[sample_token]):
            ...
            if gt_box.valid: 
                ...
                gt_size = list(gt_box.size)
                gt_yaw = quaternion_yaw(Quaternion(gt_box.rotation))
                class_id = class_inds[gt_box.detection_name]
                gt_dict[sample_token].append(gt_translation+gt_size+[gt_yaw, gt_box.index, class_id])
...
def pts_iou_match(accumulate_dict: dict, 
                  gt_boxes: EvalBoxes, 
                  pred_boxes_list: list, 
                  class_name: str,
                  cfg: DetectionConfig):
    ...
    iou_matrix = pickle.load(open(cfg.iou_matrix_path, 'rb'), encoding='iso-8859-1') 

    for sample_token in pred_dict.keys():
        pred_matrix = np.array(pred_dict[sample_token])
        gt_matrix = np.array(gt_dict[sample_token])
        try:
            iou_infos = iou_matrix[sample_token]['iou_matrix']
            det_mask = iou_matrix[sample_token]['det_mask']
        except:
            iou_infos = np.zeros((pred_matrix.shape[0], 1), dtype=pred_matrix.dtype)
            det_mask = np.ones((pred_matrix.shape[0], 1), dtype=pred_matrix.dtype) == 1
...
def accumulate(sensor: str,
    ...
    for sample_token in accumulate_dict['sample_token'].keys():
        ...
        if len(iou_metric.shape) < 3:  #该时间段没有GT
            ...
            for pred_idx in range(pred_matrix.shape[0]):
                ...
                if is_match:
                    if cfg.dist_type == 'pts_dist':
                        matched_gt_dist = dist_infos[sample_token]['gt_dists'][int(match_gt_box[7])]
                    else:
                        matched_gt_dist = np.linalg.norm(np.array(match_gt_box[:2])) 
                    ...
                    if matched_gt_dist >= distance[0] and matched_gt_dist < distance[1]:
                        ...
                        if sample_token in pred_taken.keys():
                            pred_taken[sample_token].append(global_ind)
                        else:
                            pred_taken.update({sample_token: [global_ind]})

                        tp[det_index]+=1

                        # Since it is a match, update match data also.
                        match_data['trans_err'].append(dist_metric[0, pred_idx, match_gt_idx])
                        match_data['scale_err'].append(1 - scale_iou(match_gt_box[:7], pred_box))
                        period = np.pi if class_name == 'barrier' else 2 * np.pi
                        match_data['orient_err'].append(yaw_diff(match_gt_box[6], pred_box[6], period=period))
                        if cfg.iou_type == 'pts_iou':
                            match_data['iou3d_err'].append(iou_metric[0, int(match_gt_box[7]), int(global_ind)])
                            match_data['ioubev_err'].append(iou_metric[0, int(match_gt_box[7]), int(global_ind)])
                        else:
                            match_data['iou3d_err'].append(iou_metric[0, pred_idx, match_gt_idx])
                            match_data['ioubev_err'].append(iou_metric[1, pred_idx, match_gt_idx])
                else:
    ...
    if pred_num == 0 and npos == 0:
        ...
        #print("tp_.len: ", len(tp_))
        #print("fp_.len: ", len(fp_))
        #print("tp_: ", tp_)
        #print("fp_: ", fp_)
    ...
    for sample_token in accumulate_dict['sample_token'].keys():
        ...
        if len(iou_metric.shape) < 3:  #该区间没有GT
            ...
            for pred_idx in range(pred_matrix.shape[0]):
                ...
                match_gt_idx_lid = None
                match_gt_idx_cam1 = None
                match_gt_idx_cam2 = None
                match_gt_box_lid = None
                match_gt_box_cam1 = None
                match_gt_box_cam2 = None
                pred_box = pred_matrix[pred_idx]
                for gt_idx in range(gt_matrix.shape[0]):
                    gt_box = gt_matrix[gt_idx]
                    if gt_idx in gt_taken:
                    ...
                    if sensor == 'cam':
                        ...
                        if this_metric_cam1 < min_metric_cam1:
                            ...
                            match_gt_idx_cam1 = gt_idx
                            match_gt_box_cam1 = gt_box.copy()

                        if this_metric_cam2 < min_metric_cam2:
                            min_metric_cam2 = this_metric_cam2
                            match_gt_idx_cam2 = gt_idx
                            match_gt_box_cam2 = gt_box.copy()
                    else:
                        if cfg.iou_type == 'pts_iou':
                        ...
                        if this_metric_lid < min_metric_lid:
                            ...
                            match_gt_idx_lid = gt_idx 
                            match_gt_box_lid = gt_box
                ...
                if sensor == 'cam':
                    if class_name in ['bigMot', 'smallMot', 'Mot']:
                        ...
                        is_match_cam2 = min_metric_cam2 < cfg.rel_dist_th_tp  
                    match_process_cam(sample_token, pred_idx, pred_box, is_match_cam1, is_match_cam2, match_gt_idx_cam1, match_gt_box_cam1, 
                                      match_gt_idx_cam2, match_gt_box_cam2, iou_metric, dist_metric, distance, cfg, gt_taken, pred_taken, 
                                      tp, fp, match_data, class_name, det_score, global_inds, inds_detname_dict, accumulate_dict, dist_infos,
                                      det_mask)
                else:
                    is_match_lid = min_metric_lid < 1 - cfg.iou_th_tp   
                    match_process_lid(sample_token, pred_idx, pred_box, is_match_lid, match_gt_idx_lid, match_gt_box_lid, 
                                      iou_metric, dist_metric, distance, cfg, gt_taken, pred_taken, tp, fp,match_data, 
                                      class_name, det_score, global_inds, inds_detname_dict, accumulate_dict, dist_infos, det_mask)
...
def match_process_cam(sample_token: str, 
                      ...
                      is_match_cam1: bool, 
                      is_match_cam2: bool, 
                      match_gt_idx_cam1: int, 
                      match_gt_box_cam1: np.array, 
                      match_gt_idx_cam2: int, 
                      match_gt_box_cam2: np.array, 
                      iou_metric: np.array, 
    ...
    if is_match_cam1 and is_match_cam2:
        if cfg.dist_type == 'pts_dist':
            matched_gt_dist_cam1 = dist_infos[sample_token]['gt_dists'][int(match_gt_box_cam1[7])]
            matched_gt_dist_cam2 = dist_infos[sample_token]['gt_dists'][int(match_gt_box_cam2[7])]
        else:
            matched_gt_dist_cam1 = np.linalg.norm(np.array(match_gt_box_cam1[:2]))
            matched_gt_dist_cam2 = np.linalg.norm(np.array(match_gt_box_cam2[:2]))
        ...
        if (matched_gt_dist_cam1 <= cfg.rel_ab_th and matched_gt_dist_cam2 > cfg.rel_ab_th)\
            ...
            ab_dist1 = dist_metric[0, pred_idx, match_gt_idx_cam1]
            ab_dist2 = dist_metric[0, pred_idx, match_gt_idx_cam2]
            if ab_dist1 <= ab_dist2:
                match_gt_idx = match_gt_idx_cam1
                match_gt_box = match_gt_box_cam1
            else:
                match_gt_idx = match_gt_idx_cam2
                match_gt_box = match_gt_box_cam2

            gt_taken.append(match_gt_idx)
            if sample_token in pred_taken.keys():
                pred_taken[sample_token].append(global_inds[pred_idx])
            else:
                pred_taken.update({sample_token: [global_inds[pred_idx]]})

            tp[det_index]+=1

            # Since it is a match, update match data also.
            match_data['trans_err'].append(dist_metric[0, pred_idx, match_gt_idx])
            match_data['scale_err'].append(1 - scale_iou(match_gt_box[:7], pred_box[:7]))
            # Barrier orientation is only determined up to 180 degree. (For cones orientation is discarded later)
            period = np.pi if class_name == 'barrier' else 2 * np.pi
            match_data['orient_err'].append(yaw_diff(match_gt_box[6], pred_box[6], period=period))

            if cfg.iou_type == 'pts_iou':
                match_data['iou3d_err'].append(iou_metric[0, int(match_gt_box[7]), int(global_inds[pred_idx])])
                match_data['ioubev_err'].append(iou_metric[0, int(match_gt_box[7]), int(global_inds[pred_idx])])
            else:
                match_data['iou3d_err'].append(iou_metric[0, pred_idx, match_gt_idx])
                match_data['ioubev_err'].append(iou_metric[1, pred_idx, match_gt_idx])
    ...
    if match_gt_idx is None:
        if is_match_cam1:
            ...
            if cfg.iou_type == 'pts_iou' and det_mask[int(global_inds[pred_idx])] == False:
                return
            if cfg.cross_cls_match and inds_detname_dict[sample_token][global_inds[pred_idx]] != class_name:
                return
            if cfg.cross_cls_match:
                if class_name in ['bigMot', 'smallMot', 'Mot']:
                    if distance[-1] == cfg.rel_ab_th:
                ...
                if global_inds[pred_idx] in best_match_indx and best_match_value[best_match_indx.index(global_inds[pred_idx])] <= threshold:
                    return
            
            if cfg.dist_type == 'pts_dist':
                pred_box_dist = dist_infos[sample_token]['pred_dists'][int(global_inds[pred_idx])]
            else:
                pred_box_dist = np.linalg.norm(pred_box[:2])
            if pred_box_dist <= cfg.rel_ab_th:
                fp[det_index]+=1
        ...
        if is_match_cam2: 
            ...
            if cfg.dist_type == 'pts_dist':
                ...
                pred_box_dist = np.linalg.norm(pred_box[:2])
            if pred_box_dist > cfg.rel_ab_th and pred_box_dist <= distance[1]:
                fp[det_index]+=1
...
def match_process_lid(sample_token: str, 
                      ...
                      pred_box: np.array, 
                      is_match_lid: bool, 
                      match_gt_idx_lid: int, 
                      match_gt_box_lid: np.array, 
                      iou_metric: np.array, 
                      dist_metric: np.array, 
                      distance: list, 
                      cfg: DetectionConfig, 
    ...
    if is_match_lid:
        if cfg.dist_type == 'pts_dist':
            matched_gt_dist = dist_infos[sample_token]['gt_dists'][int(match_gt_box_lid[7])]
        else:
            matched_gt_dist = np.linalg.norm(np.array(match_gt_box_lid[:2])) 
            
        if matched_gt_dist >= distance[0] and matched_gt_dist < distance[1]:
            gt_taken.append(match_gt_idx_lid)
            if sample_token in pred_taken.keys():
                pred_taken[sample_token].append(global_inds[pred_idx])
            else:
                pred_taken.update({sample_token: [global_inds[pred_idx]]})
            ...
            # Since it is a match, update match data also.
            match_data['trans_err'].append(dist_metric[0, pred_idx, match_gt_idx_lid])
            match_data['scale_err'].append(1 - scale_iou(match_gt_box_lid[:7], pred_box[:7]))

            # Barrier orientation is only determined up to 180 degree. (For cones orientation is discarded later)
            period = np.pi if class_name == 'barrier' else 2 * np.pi
            match_data['orient_err'].append(yaw_diff(match_gt_box_lid[6], pred_box[6], period=period))

            if cfg.iou_type == 'pts_iou':
                match_data['iou3d_err'].append(iou_metric[0, int(match_gt_box_lid[7]), int(global_inds[pred_idx])])
                match_data['ioubev_err'].append(iou_metric[0, int(match_gt_box_lid[7]), int(global_inds[pred_idx])])
            else:
                match_data['iou3d_err'].append(iou_metric[0, pred_idx, match_gt_idx_lid])
                match_data['ioubev_err'].append(iou_metric[1, pred_idx, match_gt_idx_lid])
    else:
        # No match. Mark this as a false positive.
        if cfg.iou_type == 'pts_iou' and det_mask[int(global_inds[pred_idx])] == False:
            return
    ...
    for sample_token in binary_accumulate_dict['sample_token'].keys():
        ...
        if len(iou_metric.shape) < 3:  #该区间没有GT
            ...
            for pred_idx in range(pred_matrix.shape[0]):
                ...
                match_gt_idx_lid = None
                match_gt_idx_abd = None
                match_gt_idx_iou = None
                match_gt_box_lid = None
                match_gt_box_abd = None
                match_gt_box_iou = None
                pred_box = pred_matrix[pred_idx]
                for gt_idx in range(gt_matrix.shape[0]):
                    gt_box = gt_matrix[gt_idx]
                    if gt_idx in gt_taken:
                    ...
                    if sensor == 'cam':
                        ...
                        if this_metric_abd < min_metric_abd:
                            ...
                            match_gt_idx_abd = gt_idx
                            match_gt_box_abd = gt_box.copy()

                        if this_metric_iou < min_metric_iou:
                            min_metric_iou = this_metric_iou
                            match_gt_idx_iou = gt_idx
                            match_gt_box_iou = gt_box.copy()
                    else:
                        if cfg.iou_type == 'pts_iou':
                            this_metric_lid = iou_metric[0, int(gt_box[7]), int(global_ind)]
                        else:
                ...
                if sensor == 'cam':
                    if len(gt_taken) == gt_matrix.shape[0]:
                        pred_box_dist = np.linalg.norm(pred_box[:2])
                        if pred_box_dist >= distance[0] and pred_box_dist < distance[1]:
                            det_index = (det_score[pred_idx] / 0.0001).astype(int)
                            fp[det_index]+=1
                        ...
                        is_match = False
                        if is_match_iou:
                            is_match = binary_match_tpfps_all_iou(sample_token, min_metric_iou, pred_idx, match_gt_idx_iou, 
                                                                  pred_box, match_gt_box_iou, tp, fp, match_data, distance, 
                                                                  cfg, iou_metric, dist_metric, det_score, gt_taken, pred_taken, 
                                                                  det_class_id, gt_class_id, inds_class, global_inds, dist_infos,
                                                                  det_mask)
                        if is_match is False:
                            binary_match_tpfps_all_abd(sample_token, min_metric_abd, pred_idx, match_gt_idx_abd, pred_box, 
                                                       match_gt_box_abd, tp, fp, match_data, distance, cfg, iou_metric, dist_metric, det_score, gt_taken,
                                                       pred_taken, det_class_id, gt_class_id, inds_class, global_inds, dist_infos,
                                                       det_mask)
                else:
                    is_match = min_metric_lid < 1 - cfg.iou_th_tp
                    binary_match_tpfps_all_lid(sample_token, min_metric_lid, pred_idx, match_gt_idx_lid, pred_box, 
                                               match_gt_box_lid, tp, fp, match_data, distance, cfg, iou_metric, 
                                               dist_metric, det_score, gt_taken, pred_taken, det_class_id, gt_class_id, 
                                               inds_class, is_match, global_inds, dist_infos,
                                               det_mask)
...
def binary_match_tpfps(sample_token: str,
    ...
    for pred_idx in range(pred_matrix.shape[0]):
        ...
        for gt_idx in range(gt_matrix.shape[0]):
            ...
            if gt_class_name in match_type and gt_idx not in gt_taken:
                if sensor == 'cam':
        ...
        if is_match:
            ...
            if matched_gt_dist >= distance[0] and matched_gt_dist < distance[1]:
                ...
                period = 2 * np.pi
                match_data['orient_err'].append(yaw_diff(match_gt_box[6], pred_box[6], period=period))
                match_data['confusion_matrix'][int(det_class_id[pred_idx]), int(gt_class_id[match_gt_idx])] += 1
...
def binary_match_tpfps_all_iou(sample_token, min_metric, pred_idx, match_gt_idx, pred_box, match_gt_box, 
                               tp, fp, match_data, distance, cfg, iou_metric, dist_metric, det_score, gt_taken,
                               pred_taken, det_class_id, gt_class_id, inds_class, global_inds,
                               dist_infos, det_mask):
    if cfg.dist_type == 'pts_dist':
        matched_gt_dist = dist_infos[sample_token]['gt_dists'][int(match_gt_box[7])]
    else:
    ...
    if matched_gt_dist <= cfg.rel_ab_th:
        if inds_class[gt_class_id[match_gt_idx]] in ['bigMot', 'smallMot', 'Mot']:
            is_match = min_metric < 1 - cfg.iou_th_tp
        else:
            is_match = dist_metric[0, pred_idx, match_gt_idx] < cfg.ab_dist_th_tp \
                                          if match_gt_idx is not None else False
    elif matched_gt_dist <= distance[1]:
        is_match = dist_metric[1, pred_idx, match_gt_idx] < cfg.rel_dist_th_tp \
                                          if match_gt_idx is not None else False
    else:
        is_match = False
...
def binary_match_tpfps_all_abd(sample_token, min_metric, pred_idx, match_gt_idx, pred_box, match_gt_box, 
                               tp, fp, match_data, distance, cfg, iou_metric, dist_metric, det_score, gt_taken,
    ...
    if matched_gt_dist <= cfg.rel_ab_th:
        ...
        if is_match:
            ...
            if cfg.dist_type == 'pts_dist':
                ...
                pred_box_dist = np.linalg.norm(pred_box[:2])
            if pred_box_dist <= distance[1]:
                fp[det_index]+=1

def binary_match_tpfps_all_lid(sample_token, min_metric, pred_idx, match_gt_idx, pred_box, match_gt_box, 
                               tp, fp, match_data, distance, cfg, iou_metric, dist_metric, det_score, gt_taken,
