...
// __global__ void ScatterMaxForwardKernel(const float* src_data,
//                                         const long* index_data,
//                                         float* out_data, int C_in, int N_out,
//                                         int numel) {
//   CUDA_1D_KERNEL_LOOP(thread_idx, numel) {
//     int index_idx = thread_idx / C_in;
//     int channel_idx = thread_idx - C_in * index_idx;
...
// template <>
// __global__ void ScatterMaxArgKernel(const paddle::float16* src_data,
//                                     const long* index_data,
//                                     const paddle::float16* out_data,
//                                     long* arg_out_data, int C_in, int N_out,
//                                     int numel) {
//   // #if __CUDA_ARCH__ >= 700 && !defined(__CUDA_ARCH__)
//   CUDA_1D_KERNEL_LOOP(thread_idx, numel) {
...
std::vector<paddle::Tensor> scatter_max_launcher(const paddle::Tensor& src, const paddle::Tensor& index, int N_in, int C_in, int N_out) {
  auto out = paddle::full({N_out, C_in}, 0.0, src.type(), paddle::GPUPlace());
  auto arg_out = paddle::full({N_out, C_in}, index.shape()[0], index.type(), paddle::GPUPlace());
  const long* index_data = index.data<long>();
  long* arg_out_data = arg_out.data<long>();
