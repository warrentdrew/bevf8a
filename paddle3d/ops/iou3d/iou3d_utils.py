...
def boxes3d_to_near_torch(boxes3d):
    ...
    boxes_center = paddle.where(cond, rboxes[:, [0, 1, 3, 2]], rboxes[:, :4])
...
def boxes_distance(points,
    ...
    if valid_dists_mask.shape[0] != 0:
        dists = dists.put_along_axis(indices=valid_dists_mask, value=valid_dists, axis=0)
    if invalid_dists_mask.shape[0] != 0:
        dists = dists.put_along_axis(indices=invalid_dists_mask, value=0.0, axis=0)
    # dists[valid_dists_mask] = valid_dists
    # dists[invalid_dists_mask] = 0.0
...
def boxes_iou_bev(boxes_a, boxes_b):
    ...
    iou3d_nms_cuda.boxes_iou_bev_gpu(boxes_a_bev, boxes_b_bev, ans_iou) #  custom op already implemented in paddle3d
...
def anchors_match_valid_voxels(anchors, voxel_mask):
    ...
    :return: anchors_mask
    """
    anchors_mask = paddle.zeros((anchors.shape[0], ), dtype = anchors.dtype).cast("bool") 
    ## TODO custom op not implemented, add it into iou3d_nms_cuda
    iou3d_nms_cuda.anchors_mask_of_valid_voxels(anchors, voxel_mask, anchors_mask) 
    return anchors_mask 
