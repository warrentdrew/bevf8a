...
__device__ inline float box_overlap_bev(const float *box_a, const float *box_b) {
  ...
  for (int k = 0; k < 4; k++) {
    ...
    printf("before corner %d: a(%.3f, %.3f), b(%.3f, %.3f) \n", k,
           ...
           box_b_corners[k].y);
#endif
    rotate_around_center_bev(center_a, a_angle_cos, a_angle_sin, box_a_corners[k]);
    rotate_around_center_bev(center_b, b_angle_cos, b_angle_sin, box_b_corners[k]);
#ifdef DEBUG
  ...
  // check corners
  for (int k = 0; k < 4; k++) {
    if (check_in_box2d_bev(box_a, box_b_corners[k])) {
...
__global__ void boxes_overlap_bev_kernel(const int num_a, const float *boxes_a,
...
void BoxesOverlapBevLauncher(const cudaStream_t &stream, const int num_a,
  ...
  boxes_overlap_bev_kernel<<<blocks, threads, 0, stream>>>(num_a, boxes_a, num_b,
                                                       boxes_b, ans_overlap);
#ifdef DEBUG
