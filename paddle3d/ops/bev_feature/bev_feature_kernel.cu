...
__global__ void MapKernel(const int n,
                          const int point_dim,
                          const float* pc,
...
__global__ void TopIntensityKernel(const int n, 
                                  const int point_dim,
                                  const float* pc, 
                                  const int* point2grid,
                                  float* top_intensity,
...
// vis feature
// logodds func2
__global__ void VoxelTraversalKernel(const int cloud_size, 
                                    const int point_dim,
                                    ...
                                    const float ray_start_x,
                                    const float ray_start_y,
                                    const float ray_start_z,
                                    float lo_occupied,
                                    float lo_free,
                                    const float* pc, 
                                    float* logodds) {
  CUDA_KERNEL_LOOP(i, cloud_size) {
    ...
    float next_voxel_boundary_x = (current_voxel_x + stepX) * voxel_size_x; // correct
    float next_voxel_boundary_y = (current_voxel_y + stepY) * voxel_size_y; // correct
    float next_voxel_boundary_z = (current_voxel_z + stepZ) * voxel_size_z; // correct

    // tMaxX, tMaxY, tMaxZ -- distance until next intersection with voxel-border
    // the value of t at which the ray crosses the first vertical voxel boundary
    float tMaxX = (ray_x != 0) ? (next_voxel_boundary_x - ray_start_x) / ray_x : FLT_MAX; //
    float tMaxY = (ray_y != 0) ? (next_voxel_boundary_y - ray_start_y) / ray_y : FLT_MAX; //
    float tMaxZ = (ray_z != 0) ? (next_voxel_boundary_z - ray_start_z) / ray_z : FLT_MAX; //
    ...
    bool neg_ray=false;
    if (current_voxel_x != last_voxel_x && ray_x < 0) { diff_x--; neg_ray=true; }
    if (current_voxel_y != last_voxel_y && ray_y < 0) { diff_y--; neg_ray=true; }
    if (current_voxel_z != last_voxel_z && ray_z < 0) { diff_z--; neg_ray=true; }

    // ray casting loop
    bool truncated = false;
    int voxel_idx = current_voxel_z * G2 + current_voxel_y * G1 + current_voxel_x;
    // debug qiaqia
    ...
    while (!truncated && 
          !(current_voxel_x == last_voxel_x && current_voxel_y == last_voxel_y && current_voxel_z == last_voxel_z)) {
      if (tMaxX < tMaxY) {
        if (tMaxX < tMaxZ) {
...
void mapfeatureLauncher(const cudaStream_t& stream,
    ...
    MapKernel<<<blocks, threads, 0, stream>>>(num, 
                                  point_dim,
                                  points_data, 
                                  points2grid_data, 
                                  max_height_data, 
...
void TopIntensityLauncher(const cudaStream_t& stream,
    ...
    dim3 blocks(DIVUP(num, THREADS_PER_BLOCK));  // blockIdx.x(col), blockIdx.y(row)
    dim3 threads(THREADS_PER_BLOCK);
    TopIntensityKernel<<<blocks, threads, 0, stream>>>(num,  
                                            point_dim,
                                            points_data,
                                            points2grid_data,
                                            top_intensity_data,
...
void ComputeLogOddsGPULauncher(const cudaStream_t& stream,
    ...
    VoxelTraversalKernel<<<blocks, threads, 0, stream>>>(num, 
                                              point_dim,
                                              voxel_size_x,
                                              voxel_size_y,
                                              voxel_size_z,
