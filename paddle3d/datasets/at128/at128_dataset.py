...
class AT128Dataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        self.cam_orders = ['obstacle', 'spherical-left-forward','spherical-right-forward',
                            'spherical-backward', 'spherical-left-backward', 'spherical-right-backward']
        ...
        self.NameMapping = {
            ...
            'cluster': 'pedestrian',
            'bigMot': 'bigMot',
            'verybigMot': 'verybigMot',
            'motorcyclist': 'nonMot',
            'bicyclist': 'nonMot',
            'OnlyTricycle': 'nonMot',
            'OnlyBicycle': 'OnlyBicycle',
            'Tricyclist': 'nonMot',
            'nonMot':'nonMot',
            'TrafficCone': 'TrainedOthers',
            'smallMovable': 'TrainedOthers',
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            ...
            for cam_type in self.cam_orders:
                ...
                lidar2cam = cam_info['lidar2cam_dynamic'] 
    ...
    def prepare_train_data(self, index):
        ...
        # input_dict.keys() = ['sample_idx', 'pts_filename', 'sweeps', 'timestamp', 'can_bus', 
        # 'img_filename', 'lidar2img', 'caminfo', 'ann_info'] # for nuscens
    ...
    def get_ann_info(self, index):
        ...
        #dict_keys(['gt_boxes', 'gt_names', 'timestamp', 'cams', 'valid_flag'])
        # filter out bbox containing no points
        if self.use_valid_flag:
            mask = info['valid_flag']
            # gt_bboxes_3d = info['gt_boxes'][mask]
            # gt_names_3d = info['gt_names'][mask]
            gt_bboxes = info['gt_boxes'][mask]
        ...
        gt_bboxes_3d = []
        gt_names_3d = []
        for each_box, each_name in zip(gt_bboxes, gt_names):
            mapped_name = self.NameMapping[each_name]
            if mapped_name == '':
                continue
            gt_bboxes_3d.append(each_box)
            gt_names_3d.append(mapped_name)
        ...
        if 'roi_data' in info:
            anns_results['roi_data'] = info['roi_data']
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(results):
            ...
            filenames[sample_token] = self.data_infos[sample_id]['cams']['spherical-right-forward']['data_path'].split('+')[0]
        nusc_submissions = {
            'meta': self.modality,
            'results': nusc_annos,
            'filenames': filenames
    ...
    def format_results(self, results, jsonfile_prefix=None):
        ...
        if not isinstance(results[0], dict):
            ...
            for name in results[0]:
                if name in ['sample_idx', 'filename']:
                    continue
    ...
    # # override collate_fn from base dataset
    def collate_fn(self, batch):
        ...
        if isinstance(sample, np.ndarray):
            ...
            # valid_keys = [key for key in valid_keys if key not in ['gt_labels_3d', 'gt_bboxes_3d']]
