...
class BEVFormerHead(nn.Layer):
    ...
    def forward(self, bev_embed, img_metas=None, prev_bev=None, only_bev=False):
        if hasattr(self, 'amp_cfg_'):
            for key, bev_embed_ in enumerate(bev_embed):
                bev_embed[key] = bev_embed_.cast(paddle.float16)

        bev_embed = bev_embed[0]
        bs = bev_embed.shape[0]
        dtype = bev_embed.dtype
        bev_embed = bev_embed.transpose([0,2,3,1]).reshape([bs, -1, self.embed_dims])
        assert bev_embed.shape[1] == self.bev_h * self.bev_w
    ...
    def _get_target_single(self,
        ...
        assign_result = self.assigner.assign(bbox_pred, cls_score, gt_bboxes,
                                             gt_labels, valid_flag, gt_bboxes_ignore)
    ...
    def get_targets(self,
        ...
        bbox_weights_list, pos_inds_list, neg_inds_list = multi_apply(
            ...
            gt_labels_list, gt_bboxes_list, valid_flag_list, gt_bboxes_ignore_list)
    ...
    def loss_single(self,
        ...
        cls_reg_targets = self.get_targets(cls_scores_list, bbox_preds_list,
                                           gt_bboxes_list, gt_labels_list, valid_flag_list,
    ...
    def loss(self,
        ...
        all_valid_flag_list = None
        if valid_flag:
            all_valid_flag_list = [valid_flag for _ in range(num_dec_layers)]
        else:
            tmp_batch = []
            for idx in range(len(gt_labels_list)):
                tmp = [True,] * gt_labels_list[idx].shape[0]
                tmp_batch.append(tmp)
            all_valid_flag_list = [tmp_batch for _ in range(num_dec_layers)]
        ...
        losses_cls, losses_bbox = multi_apply(
            ...
            all_gt_bboxes_list, all_gt_labels_list, all_valid_flag_list, all_gt_bboxes_ignore_list)
