...
def boxes_iou3d_gpu(boxes_a, boxes_b, return_bev=False):
    """
    Args:
    ...
    # bev iou
    area_a = (boxes_a[:, 3] * boxes_a[:, 4]).reshape([-1, 1])  # (N, 1)
    area_b = (boxes_b[:, 3] * boxes_b[:, 4]).reshape([1, -1])  # (1, M)  -> broadcast (N, M)
    iou_bev = overlaps_bev / paddle.clip(area_a + area_b - overlaps_bev, min=1e-7)
