...
class BEVFormerEncoder(nn.Layer):
    ...
    def point_sampling(self, reference_points, point_cloud_range, img_metas, lidar_aug_matrix=None):
        ...
        # lidar aug matrix (B, 4, 4)
        if lidar_aug_matrix is not None:
            for i in range(lidar_aug_matrix.shape[0]):
                lidar_aug_matrix[i] = paddle.linalg.inv(lidar_aug_matrix[i])
            lidar_aug_matrix = lidar_aug_matrix.reshape([lidar2img.shape[0], 1, 
                                        4, 4]).repeat([1, lidar2img.shape[1], 1, 1])
            lidar2img = paddle.matmul(lidar2img, lidar_aug_matrix)
    ...
    def forward(self,
        ...
        reference_points_cam, bev_mask = self.point_sampling(
            ref_3d, self.point_cloud_range, kwargs['img_metas'], lidar_aug_matrix=lidar_aug_matrix)

        # bug: this code should be 'shift_ref_2d = ref_2d.clone()', we keep this bug for reproducing our results in paper.
        # TODO(qianhui): fix this clone bugs: paddle equal means clone but paddle not
        #shift_ref_2d = ref_2d
