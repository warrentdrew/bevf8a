...
@manager.BBOX_ASSIGNERS.add_component
class HungarianAssigner3DFromBEVFormer(object):
    """Computes one-to-one matching between predictions and ground truth.
    This class computes an assignment between the targets and the predictions
    based on the costs. The costs are weighted sum of three components:
    classification cost, regression L1 cost and regression iou cost. The
    ...
    def assign(self,
        ...
        # 1. assign -1 by default
        assigned_gt_inds = paddle.full((num_bboxes, ),
                                              -1,
                                              dtype='int64')
        assigned_labels = paddle.full((num_bboxes, ),
                                             -1,
                                             dtype='int64')
        if num_gts == 0 or num_bboxes == 0:
            # No ground truth or boxes, return empty assignment
        ...
        if valid_flag is not None:
            for inds in range(matched_col_inds.shape[0]):
                matched_row_inds_i = matched_row_inds[inds]
                matched_col_inds_i = matched_col_inds[inds]
                if valid_flag[matched_col_inds_i] == False:
                    assigned_gt_inds = paddle.scatter(assigned_gt_inds, matched_row_inds_i, -1)
                else:
                    assigned_gt_inds = paddle.scatter(assigned_gt_inds, matched_row_inds_i,
                                          matched_col_inds_i + 1)
                    assigned_labels = paddle.scatter(
                                        assigned_labels, matched_row_inds_i,
                                        paddle.gather(gt_labels, matched_col_inds_i, axis=0))
        else:
            assigned_gt_inds = paddle.scatter(assigned_gt_inds, matched_row_inds,
                                          matched_col_inds + 1)
            assigned_labels = paddle.scatter(
