...
def build_norm_layer(cfg, num_features, postfix='', init_val=1, norm_decay=0.0):
    ...
    cfg_['epsilon'] = 1e-5
    weight_attr = ParamAttr(initializer=Constant(value=init_val), regularizer=L2Decay(norm_decay))
    bias_attr = ParamAttr(initializer=Constant(value=0), regularizer=L2Decay(norm_decay))
...
class BEVFormerLayer(nn.Layer):
    ...
    def __init__(self,
        ...
        for _ in range(num_norms):
            self.norms.append(build_norm_layer(norm_cfg, self.embed_dims, norm_decay=norm_decay)[1])
